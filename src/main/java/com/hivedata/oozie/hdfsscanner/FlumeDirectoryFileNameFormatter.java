...
public class FlumeDirectoryFileNameFormatter implements FileNameFormatter {
    // hdfs://localhost:8020/flume/test/2013/05/09/00/08/FlumeData.1368058096702
    ...
    public FileMeta parse(String filePath) {
        ...
        try {
             ...
             */
            String minute = args[args.length -1];
            String hour = args[args.length -2];
            String day = args[args.length -3];
            String month = args[args.length -4];
            String year = args[args.length -5];
